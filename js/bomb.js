...
Bomb.prototype.kill_players = function() {
    ...
    var player_x = bitmap_position(player[playerid].x+player[playerid].sprite_width/2);
    var player_y = bitmap_position(player[playerid].y+player[playerid].sprite_height/2);

    if (player_y == bomb_y || player_x == bomb_x) {     // player on the same x-axis or y-axis as the bomb
        if ((player_x >= bomb_x-1*this.radius["left"] && player_x <= bomb_x+1*this.radius["right"]) &&(player_y <= bomb_y+1*this.radius["down"] && player_y >= bomb_y-1*this.radius["up"])){ // player within the radius of the explosion
            player[playerid].kill();
            send("|"+playerid+player[playerid].x+","+player[playerid].y);
        }
    }
    board.level[bitmap_position(this.y)][bitmap_position(this.x)] = 0;
    //SendGameState();
}
